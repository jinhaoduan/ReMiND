...
def anneal_dsm_score_estimation(scorenet, x, labels=None, loss_type='a', hook=None, cond=None, cond_mask=None, gamma=False, L1=False, all_frames=False):
    ...
    with torch.cuda.amp.autocast():
        scorenet_out = scorenet(perturbed_x, labels, cond_mask=cond_mask)
        loss = pow_((z - scorenet_out).reshape(len(x), -1)).sum(dim=-1)
...
def autoencoder_mse(scorenet, x, y, labels=None, loss_type='a', hook=None, cond=None, cond_mask=None, gamma=False, L1=False, all_frames=False):
    ...
    if L1:
        ...
        def pow_(x):
            return 1 / 2. * x.square()
    with torch.cuda.amp.autocast():
        scorenet_out = scorenet(x)
        loss = pow_((y - scorenet_out).reshape(len(x), -1)).sum(dim=-1)
